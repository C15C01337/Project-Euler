...
newarray = np.array([[int(x) for x in s.split()][i:i+20] for i in range(0,400,20)]).reshape(20, 20)
...
mr = max(np.prod(newarray[i,j:j+4]) for i in range(20) for j in range(16))

#check down: row: 1 thru 16 (index 0:15); columns: 1 thru 20 (index 0:19)
mc = max(np.prod(newarray[i:i+4,j]) for i in range(16) for j in range(20))
...
mx = max(np.prod([newarray[i+k,j+k] for k in range(4)]) for i in range(16) for j in range(16))
...
my = max(np.prod([newarray[i+k,j-k] for k in range(4)]) for i in range(16) for j in range(3,20))
